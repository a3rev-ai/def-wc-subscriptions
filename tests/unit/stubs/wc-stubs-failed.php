<?php
/**
 * WooCommerce stubs for failed-order test.
 *
 * Simulates: 1 subscription with parent (completed) + 1 renewal (failed).
 * Expected total_spent: 29.99 only — failed excluded.
 */

class Stub_WC_Date_F {
    private string $d;
    public function __construct( string $d ) { $this->d = $d; }
    public function date( string $f ): string { return $this->d; }
}

class Stub_WC_Item_F {
    public function get_name(): string { return 'Plan'; }
}

class Stub_WC_Order_F {
    private int $id;
    ...
    public function get_date_created(): Stub_WC_Date_F {
        return new Stub_WC_Date_F( '2026-01-01T00:00:00+00:00' );
    }
}
...
class Stub_WC_Sub_F {
    ...
    public function get_items(): array { return array( new Stub_WC_Item_F() ); }
...
function wcs_get_users_subscriptions( int $uid ): array {
    return array( new Stub_WC_Sub_F() );
}

function wc_get_order( int $id ) {
    return match( $id ) {
        200     => new Stub_WC_Order_F( 200, 'completed', 29.99 ),
        301     => new Stub_WC_Order_F( 301, 'failed', 29.99 ),
        default => null,
    };
}
